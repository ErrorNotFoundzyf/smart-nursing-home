...
python checkingstrangersandfacialexpression.py --filename room_01.mp4
'''
...
# 全局变量
facial_recognition_model_path = '/root/Desktop/old_care_system/models/face_recognition_hog.pickle'
facial_expression_model_path = '/root/Desktop/old_care_system/models/face_expression.hdf5'

output_stranger_path = '/root/Desktop/old_care_system/supervision/strangers'
output_smile_path = '/root/Desktop/old_care_system/supervision/smile'

people_info_path = '/root/Desktop/old_care_system/info/people_info.csv'
facial_expression_info_path = '/root/Desktop/old_care_system/info/facial_expression_info.csv'

python_path = '/root/anaconda3/bin/python3' # your python path
...
# 得到 ID->姓名的map 、 ID->职位类型的map、摄像头ID->摄像头名字的map、表情ID->表情名字的map
...
facial_expression_limit_time = 2 # if >= 2 seconds, then he/she is smiling.
...
while True:
	...
	gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)#grayscale，用于表情识别
	...
	cv2.line(frame, (one_fourth_image_center[0], 0), 
             (one_fourth_image_center[0], VIDEO_HEIGHT), (0, 255, 255), 1)
	cv2.line(frame, (three_fourth_image_center[0], 0), 
            (three_fourth_image_center[0], VIDEO_HEIGHT), (0, 255, 255), 1)
	...
	for ((left, top, right, bottom), name) in zip(face_location_list, names):
		...
		if 'Unknown' in names: # alert
			if strangers_timing == 0: # just start timing
				...
				if difference < strangers_limit_time:
					...
					direction = ''
					if unknown_face_center[0] < one_fourth_image_center[0]: # face locates too left, servo need to turn right, so that face turn right as well
					    direction = 'right'
		...
		# 表情检测逻辑
		if name != 'Unknown' and id_card_to_type[name] == 'old_people': # 如果不是陌生人，且对象是老人
		#if name != 'Unknown': # 如果不是陌生人
			# 表情检测逻辑
			...
			if facial_expression_label == 'Smile': # alert
				if facial_expression_timing == 0: # just start timing
					...
					if difference < facial_expression_limit_time:
					    ...
					    cv2.imwrite(os.path.join(output_smile_path, 'snapshot_%s.jpg' %(time.strftime('%Y%m%d_%H%M%S'))), frame)# snapshot
		...
		#draw.text((left, top - 30), final_label, font=ImageFont.truetype('../fonts/arialuni.ttf', 20), fill=(255,0,0)) # windows
	...
	cv2.imshow("Checking Strangers and Ole People's Facial Expression", frame)
