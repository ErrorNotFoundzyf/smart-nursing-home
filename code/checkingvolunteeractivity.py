...
python checkingvolunteeractivity.py --filename desk_01.mp4
'''
...
output_activity_path = '/root/Desktop/old_care_system/supervision/activity'
model_path = '/root/Desktop/old_care_system/models/face_recognition_hog.pickle'
        ...
        for i in volunteer_centroids:
            for j_index, j in enumerate(old_people_centroids):
                ...
                if actual_distance < ACTUAL_DISTANCE_LIMIT:
                    ...
                    cv2.imwrite(os.path.join(output_activity_path, 'snapshot_%s.jpg' %(time.strftime('%Y%m%d_%H%M%S'))), frame)# snapshot
